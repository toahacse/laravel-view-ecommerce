                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-6">
                                            <div class="form-grp">
                                                ...
                                                <input type="text" id="zip" @blur="getPincodeData()" v-model="pincode">
                                            </div>
                                        </div>
                                        <div class="col-12">
                                            <div class="form-grp">
    ...
    methods:{
        async placeOrder(){
            if(this.firstName !== '' && this.lastName !== '' && this.email !== '' && this.address !== ''
               && this.country !== '' && this.phone !== '' && this.paymentMethod !== '' && this.pincode !== ''){
                try {
                    let data = await axios.post(getUrlList().placeOrder,{
                        'token' : this.user_info.user_id,
                        'auth' : this.user_info.auth,
                        'firstName' : this.pincfirstNameode,
                        'pinlastNamecode' : this.lastName,
                        'email' : this.email,
                    ...
                    if(data.status == 200){
                        this.city = data.data.data.data.thana;
                        this.state = data.data.data.data.district;
                    }else{
                ...
                alert('All field are qequired!')
            }
        },
        async getPincodeData(){
            if(this.pincode !== ''){
                try {
                    let data = await axios.post(getUrlList().getPincodeData,{
                        'token' : this.user_info.user_id,
                        'auth' : this.user_info.auth,
                        'pincode' : this.pincode,
                    })
